document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayItems = async () => {
        ...
        if (movies.length === 0) movieContainer.innerHTML = '<p class="placeholder">No movies in your watchlist.</p>';
        if (tvShows.length === 0) tvContainer.innerHTML = '<p class="placeholder">No TV shows in your watchlist.</p>';
    ...
    document.querySelector('.list-page-container').addEventListener('click', (e) => {
        ...
        if (button.classList.contains('mark-watched-btn')) {
            // Move from watchlist to watchedlist
            const itemToMove = watchlist.find(i => i.id === id && i.type === type);
            if (itemToMove) {
                watchedlist.push(itemToMove);
